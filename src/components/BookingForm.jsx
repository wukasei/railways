    ...
    <InternalModal onClose={onClose}>
      {!isSuccess ? (
        ...
        <div className={styles.successContainer}>
          ...
          <h2>Квитки заброньовано!</h2>
          <p>Дякуємо, <strong>{formData.fullName}</strong>. Ваші місця чекають на вас.</p>
          <div className={styles.detailsMinimal}>
             <span>Вагон №{wagon?.number}</span>
             <span>Місця: {selectedSeats?.join(', ')}</span>
          </div>
          <button onClick={onClose} className={styles.finishBtn}>Чудово!</button>
        </div>
      )}
    </InternalModal>
  );
